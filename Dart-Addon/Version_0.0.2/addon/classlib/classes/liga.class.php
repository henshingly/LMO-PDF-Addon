 ...
 *
 * @package   classLib
 * @version  $Id: liga.class.php,v 1.5 2005/08/27 13:34:43 jokerlmo Exp $
*/
 ...
 * @param defaultV string Default Wert 
 * @return string
*/
  ...
  function loadFile($fileName='') {
    ...
    if(file_exists($fileName) and $fileName <> '') {
      ...
      while (isset($iniData["Round".$rCounter]) and $iniData["Round".$rCounter]!="" ) {
        ...
        $startDateString = $this->getIniData("D1",$iniData[$roundSektion]);//$iniData[$roundSektion]["D1"];
        $endDateString = $this->getIniData("D2",$iniData[$roundSektion]);//$iniData[$roundSektion]["D2"];
        // Neu ab Version 2.2
        $pokalModus = $this->getIniData("MO",$iniData[$roundSektion]);
//        echo "<BR>$roundSektion startDateString=$startDateString pokalModus=".$pokalModus;
        ...
        while (isset($iniData[$roundSektion]["TA".$pCounter])
          ...
          $heim =  $this->getIniData("TA".$pCounter,$iniData[$roundSektion]);
          $gast =  $this->getIniData("TB".$pCounter,$iniData[$roundSektion]);
          ...
          do {
            $ppcStr = ($ppc <> 0)?$ppc:'';
            $theim =  $this->getIniData("GA".$pCounter.$ppcStr,$iniData[$roundSektion]);
            $tgast =  $this->getIniData("GB".$pCounter.$ppcStr,$iniData[$roundSektion]);
            $notiz =  $this->getIniData("NT".$pCounter.$ppcStr,$iniData[$roundSektion]);
            $zeit =   $this->getIniData("AT".$pCounter.$ppcStr,$iniData[$roundSektion]);
            $reportUrl = $this->getIniData("BE".$pCounter.$ppcStr,$iniData[$roundSektion]);
            // Neu ab 2.2 Spielende
            $spEnde = $this->getIniData("SP".$pCounter.$ppcStr,$iniData[$roundSektion],0);
...
//        $tableArray[$gastCount]["dTor"] += $partie->gTore-$partie->hTore;
  ...
  if ($kegel==1 && $ligaType != "dart") { // Sortierung PlusPkt,erzielte Tore
        ...
        if ($ligaType == "dart" ) {  // Dart Liga sort -> Punkte/Tordifferenz/gewonnene Spiele / verlorene Spiele / differenz Sätze / gewonnene Sätze
          $sort_5[] = $table["dsatz"];
          $sort_6[] = $table["psatz"];
