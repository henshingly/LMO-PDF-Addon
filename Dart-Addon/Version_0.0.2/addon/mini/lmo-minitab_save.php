...
if(($_SESSION['lmouserok']==2)||($_SESSION['lmouserok']==1)){
  ...
  if ($lmtype==0) {
    ...
    foreach ($tab0 as $y) {
      ...
      if ($minus == 2) {
        fwrite($fp,applyFactor($negativ[$tabledata[$teamNumber]-50000000],$pointsfaktor));   //Points-
      }
      fwrite($fp,'|');
      ...
      fwrite($fp,'|'.$teamm[$tabledata[$teamNumber]-50000000]);           //TeamShortName (Mittellanger Name)
 // Dart Liga      
      if (isset($psaetze) ) {
