  ...
  if ($save == 1) {
    ...
    for($i = 0; $i < $anzsp2; $i++) {
      ...
      if ($lmtype == 0) {
        ...
        // Dart Liga Hack Anfang
        $satza[$st-1][$i] = isset($_POST["xsatza".$i]) ? trim($_POST["xsatza".$i]) : '';
        if ($satza[$st-1][$i] == "" || $satza[$st-1][$i] == "0") {
          $satza[$st-1][$i] = 0;
        } else {
          $satza[$st-1][$i] = intval(trim($satza[$st-1][$i]));
          if ($satza[$st-1][$i] == "") {
            $satza[$st-1][$i] = "0";
          }
        }
        $satzb[$st-1][$i] = isset($_POST["xsatzb".$i]) ? trim($_POST["xsatzb".$i]) : '';
        if ($satzb[$st-1][$i] == "" || $satzb[$st-1][$i] == "0") {
          $satzb[$st-1][$i] = 0;
        } else {
          $satzb[$st-1][$i] = intval(trim($satzb[$st-1][$i]));
          if ($satzb[$st-1][$i] == "") {
            $satzb[$st-1][$i] = "0";
          }
        }
        ...
        for($n = 0; $n < $modus[$st-1]; $n++) {
         ...
         // Dart Liga Hack Anfang
         $satza[$st-1][$i][$n] = trim($_POST["xsatza".$i.$n]);
          if ($satza[$st-1][$i][$n] == "" || $satza[$st-1][$i][$n] == "0") {
            $satza[$st-1][$i][$n] = 0;
          } else {
            $satza[$st-1][$i][$n] = intval(trim($satza[$st-1][$i][$n]));
            if ($satza[$st-1][$i][$n] == "") {
              $satza[$st-1][$i][$n] = 0;
            }
          }
         $satzb[$st-1][$i][$n] = trim($_POST["xsatzb".$i.$n]);
          if ($satzb[$st-1][$i][$n] == "" || $satzb[$st-1][$i][$n] == "0") {
            $satzb[$st-1][$i][$n] = 0;
          } else {
            $satzb[$st-1][$i][$n] = intval(trim($satzb[$st-1][$i][$n]));
            if ($satzb[$st-1][$i][$n] == "") {
              $satzb[$st-1][$i][$n] = 0;
            }
          }
...
<table class="lmoMiddle" cellspacing="0" cellpadding="0" border="0">
    ...
    <td align="center">
      <form name="lmoedit" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" onSubmit="return chklmopass()">
        ...
        <table class="lmoInner" cellspacing="0" cellpadding="0" border="0">
          <tr>
            <th class="nobr" align="left" colspan="<?php echo $breite-12; ?>"><?php
  echo $st.". ".$text[2];
  if ($datum1[$st-1] != "") {
    $datum = explode('.', $datum1[$st-1]);
  ...
  if ($goalfaktor!=1) {?>
            <th class="nobr" colspan="<?php echo $breite-11; ?>"><?php if ($goalfaktor!=1) {echo "(".$text[553+log10($goalfaktor)].")";}?></th><?php
  } else {?>
            <th colspan="<?php echo $breite-11; ?>"><?php echo $text[38]?></th><?php
  }
// Dart Liga Hack Anfang
  if ($ligaType == "dart") :
    $DartLigaBreite = 4;
    ...
    if ($lmtype == 0) {?>
      ...
      if($goalb[$st-1][$i]=="-1"){
        $goalb[$st-1][$i]="_";
            ...
            <!-- Dart Liga  //-->
      <?php if ($ligaType == "dart" ) : ?>
            <td width="2">&nbsp;</td>
            <td class="lmoBackMarkierung" align="right"><input title="<?php echo $text['dart'][001] ?>" class="lmo-formular-input" type="text" name="xsatza<?php echo $i; ?>" size="1" maxlength="4" value="<?php echo $satza[$st-1][$i]; ?>" tabindex="<?php echo $i;?>9" onChange="lmotorgte('b','<?php echo $i; ?>')" onKeyDown="lmotorclk('b','<?php echo $i; ?>',event.keyCode)"></td>
            <td class="lmoBackMarkierung" align="center" width="8">:</td>
            <td class="lmoBackMarkierung" align="right"><input title="<?php echo $text['dart'][002] ?>" class="lmo-formular-input" type="text" name="xsatzb<?php echo $i; ?>" size="1" maxlength="4" value="<?php echo $satzb[$st-1][$i]; ?>" tabindex="<?php echo $i;?>9" onChange="lmotorgte('b','<?php echo $i; ?>')" onKeyDown="lmotorclk('b','<?php echo $i; ?>',event.keyCode)"></td>
      <?php endif;
      // Dart Liga Hack Ende
      if($spez==1){?>
            <td width="2">&nbsp;</td>
            <td>
              <select class="lmo-formular-input" name="xmspez<?php echo $i; ?>" onChange="dolmoedit()" tabindex="<?php echo $i;?>11" >
            ...
            <td width="2">&nbsp;</td>
            <td align="center">
              <select id="gT<?php echo $i?>" class="lmo-formular-input" name="xmsieg<?php echo $i; ?>" onChange="dolmoedit()" tabindex="<?php echo $i;?>12" >
      ...
      if($msieg[$st-1][$i]==0) {?>
            ...
            <td align="center">
              <input id="n<?php echo $i?>" class="lmo-formular-input" type="text" name="xmnote<?php echo $i; ?>" tabindex="<?php echo $i;?>13" size="16" maxlength="255" value="<?php echo htmlentities($mnote[$st-1][$i]); ?>" onChange="dolmoedit()"><?php
      ...
      if($_SESSION['lmouserok']==2 && $ftest0==1){ ?>
            <td>
              <select class="lmo-formular-input" name="xmtipp<?php echo $i; ?>" onChange="dolmoedit()" tabindex="<?php echo $i;?>14" >
      ...
      for($n=0;$n<$modus[$st-1];$n++){
        ...
        <?php if ($ligaType == "dart" ) : ?>
            <td width="2">&nbsp;</td>
            <td class="lmoBackMarkierung" align="right"><input title="<?php echo $text['dart'][001] ?>" class="lmo-formular-input" type="text" name="xsatza<?php echo $i.$n; ?>" size="1" maxlength="4" tabindex="<?php echo $i.$n;?>7" value="<?php echo $satza[$st-1][$i][$n]; ?>" onChange="lmotorgte('b','<?php echo $i.$n; ?>')" onKeyDown="lmotorclk('b','<?php echo $i.$n; ?>',event.keyCode)"></td>
            <td class="lmoBackMarkierung" align="center" width="8">:</td>
            <td class="lmoBackMarkierung" align="right"><input title="<?php echo $text['dart'][002] ?>" class="lmo-formular-input" type="text" name="xsatzb<?php echo $i.$n; ?>" size="1" maxlength="4" tabindex="<?php echo $i.$n;?>8" value="<?php echo $satzb[$st-1][$i][$n]; ?>" onChange="lmotorgte('b','<?php echo $i.$n; ?>')" onKeyDown="lmotorclk('b','<?php echo $i.$n; ?>',event.keyCode)"></td>
        <?php endif; ?>
            <!-- Dart Liga  //-->
